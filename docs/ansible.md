...
Run from **`ansible/`** so **`ansible.cfg`** applies: node baseline, **kubeadm**, **Calico**, optional **Flux**.
...
Order: prepare nodes (containerd, packages) → **kubeadm init** → join workers → **Calico** → checks. **`pod_network_cidr`** must match kubeadm. Role layout: **`ansible/roles/kubernetes/`**.
...
Example vars: **`ansible/playbooks/vars/fluxcd-github.example.yml`**. **`fluxcd_github_bootstrap: true`** runs **`flux bootstrap github`** (not **`flux install`**).
...
- Workers without internet need NAT/proxy (**[terraform.md](terraform.md)**, **`terraform/modules/kubernetes/EGRESS.md`**).  
