...
2. Define an `ObjectStore` that is the S3 location Barman Cloud reads and writes [Example](gitops/applications/base/postgres-cluster/objectstore.yaml).
3. On the `Cluster`, set `spec.plugins` like the [Example](gitops/applications/base/postgres-cluster/cluster.yaml).
4. Create `ScheduledBackup` with `method: plugin` like the example [Example](gitops/environments/dev/demo-app/cnpg-scheduledbackup.yaml).
...
This sequence is done automatically by the Barman Plugin.

**Operational sequence:** 
1. stop application writers
2. define a new or replacement **Cluster** with **bootstrap.recovery** per **[CNPG recovery](https://cloudnative-pg.io/docs/1.28/recovery)** 
3. verify
4. point the app at the DB **Secret / service**
5. resume traffic.

**How Barman Cloud fits in:** recovery still uses **bootstrap.recovery**. 

The plugin must read the same Barman layout in S3 as the backup chain: keep **spec.plugins** and an **ObjectStore** that match (same plugin, `barmanObjectName`, `destinationPath`). 

For a restore from an existing **Backup CR**, reference it under **bootstrap.recovery** (field names depend on your CNPG version). 
...
**More:** **[postgres-backup-strategy](postgres-backup-strategy.md)**, [gitops/infrastructure/postgres/BACKUP.md](gitops/infrastructure/postgres/BACKUP.md), **[Barman Cloud plugin](https://cloudnative-pg.io/plugin-barman-cloud/)**.
...
**3 — bootstrap.recovery:** **source** must be **externalClusters[].name** (here **pg17-barman-source**), not the **Clusters** metadata name:
...
The grafana was configure for this domain:


grafana.alissonmachado.com.br , but you can update it on [gitops/operators/kube-prometheus-stack/helmrelease.yaml](gitops/operators/kube-prometheus-stack/helmrelease.yaml)
