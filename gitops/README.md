...
| **`infrastructure/`** | Shared platform: **Flux** **`PodMonitor`** (after Prometheus CRDs), **Let’s Encrypt** `ClusterIssuer`, **OpenBao** **`auth/kubernetes`** bootstrap (**`openbao-kubernetes-auth/`**), **OpenBao** `Ingress`, **Traefik** `HelmRelease`, CloudNativePG **`Cluster`** (e.g. **`postgres/`**), namespaces. Synced by **`clusters/dev/infrastructure.yaml`** with **`dependsOn: operators`**. |
