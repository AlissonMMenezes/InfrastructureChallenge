...
| **`infrastructure/`** | Shared platform: **Flux** **`PodMonitor`** (after Prometheus CRDs), **Let’s Encrypt** `ClusterIssuer`, **OpenBao** **`auth/kubernetes`** bootstrap (**`openbao-kubernetes-auth/`**), **OpenBao** `Ingress`, **Traefik** `HelmRelease`, CloudNativePG **`Cluster`** + **`ScheduledBackup`** + S3 backup notes (**`postgres/`**, see **`postgres/BACKUP.md`**), namespaces. Synced by **`clusters/dev/infrastructure.yaml`** with **`dependsOn: operators`**. |
