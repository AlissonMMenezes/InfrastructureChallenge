...
Helm chart: Prometheus Operator, Prometheus, Alertmanager, Grafana, node-exporter, kube-state-metrics.
...
**OpenBao:** chart **ServiceMonitor** on metrics path when enabled in **`operators/openbao/helmrelease.yaml`**.

**Grafana:** admin password — **`docs/operations.md`**. Ingress host in Helm values (see **`docs/gitops.md`**).
